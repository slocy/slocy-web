...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'BIGPIX-install-plugins';
         ...
         * does the BIGPIX_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'oops'                           => __( 'Something went wrong.', 'BIGPIX' ),
            ...
            do_action_ref_array( 'BIGPIX_init', array( $this ) );
         ...
         *
         * We get the links in the admin notices to point to the BIGPIX page, rather
         ...
         * @return null Returns early if not the BIGPIX page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'BIGPIX_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new BIGPIX_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'BIGPIX-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="BIGPIX wrap">
                ...
                <form id="BIGPIX-plugins" action="" method="post">
                    <input type="hidden" name="BIGPIX-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['BIGPIX-install'] ) && 'install-plugin' == $_GET['BIGPIX-install'] ) ) {
                check_admin_referer( 'BIGPIX-install' );
                ...
                $fields = array( 'BIGPIX-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'BIGPIX' ) . '">' . __( 'Return to the Dashboard', 'BIGPIX' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['BIGPIX-activate'] ) && 'activate-plugin' == $_GET['BIGPIX-activate'] ) ) {
                check_admin_referer( 'BIGPIX-activate', 'BIGPIX-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'BIGPIX' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'BIGPIX' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'BIGPIX' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'BIGPIX_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'BIGPIX-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'BIGPIX', 'BIGPIX', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'BIGPIX', 'BIGPIX', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'BIGPIX' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['BIGPIX-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'BIGPIX_dismissed_notice', 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the BIGPIX install page.
            if ( $this->is_BIGPIX_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the BIGPIX Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the BIGPIX page, false otherwise.
         */
        protected function is_BIGPIX_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'BIGPIX_dismissed_notice' );
    ...
    $BIGPIX = TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'BIGPIX' ) ) {
    /**
     ...
     */
    function BIGPIX( $plugins, $config = array() ) {
...
if ( ! class_exists( 'BIGPIX_List_Table' ) ) {
    /**
     ...
     */
    class BIGPIX_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the BIGPIX class
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'BIGPIX' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'BIGPIX' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'BIGPIX' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'BIGPIX' ) : __( 'Recommended', 'BIGPIX' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'BIGPIX' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'BIGPIX' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'BIGPIX' ) . ' %2$s">' . __( 'Install', 'BIGPIX' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'BIGPIX' ) . ' %2$s">' . __( 'Activate', 'BIGPIX' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'BIGPIX-activate'       => 'activate-plugin',
                                'BIGPIX-activate-nonce' => wp_create_nonce( 'BIGPIX-activate' ),
                            ),
                            network_admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'BIGPIX' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'BIGPIX-bulk-install'  => __( 'Install', 'BIGPIX' ),
                'BIGPIX-bulk-activate' => __( 'Activate', 'BIGPIX' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'BIGPIX-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => TGM_Plugin_Activation::$instance->menu,
                            'BIGPIX-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'BIGPIX-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'BIGPIX-empty';
                    $i++;
                }
            ...
            if ( 'BIGPIX-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'BIGPIX' ), $imploded );
                }
...
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
             ...
             * set to true for the BIGPIX class.
             *
             * @since 2.2.0
             *
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'BIGPIX' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'BIGPIX' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'BIGPIX' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'BIGPIX' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'BIGPIX' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'BIGPIX' );
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'BIGPIX' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'BIGPIX' ) . '</span><span class="hidden">' . __( 'Hide Details', 'BIGPIX' ) . '</span>.</a>';
                ...
                // Default installation strings.
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'BIGPIX' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'BIGPIX' ) . '</span><span class="hidden">' . __( 'Hide Details', 'BIGPIX' ) . '</span>.</a>';
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'BIGPIX' ) . '">' . __( 'Return to the Dashboard', 'BIGPIX' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
